...
def main_generator(config_file_1: Annotated[str, typer.Argument(help="Configuration file containing description of a neural network we want to generate.")],
    ...
    try:
        ...
        if len(other_configs) % 2 != 0:
            raise IndexError(
                "Invlaid number of arguments, each config must have a number")
        for i in range(0, len(other_configs), 2):
            try:
                configs.append(
                    Config(other_configs[i], int(other_configs[i + 1])))
            except ValueError:
                raise ValueError(
                    f"Error: Arg :'{other_configs[i]}' should be a valid int")
